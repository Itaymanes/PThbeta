...
class PThBeta:
    ...
    def plot_sampler_grid(self, data, pred_sample, title=None):
        ...
        if p == 2:
            ...
            ax.scatter(x=pred_sample[:, 0], y=pred_sample[:, 1], alpha=0.3, s=20, c='blue')
            ax.scatter(x=np_d[:, 0], y=np_d[:, 1], alpha=1, s=40, c='red')
            ax.set_title(title)
            ax.grid()
            ...
            ax.scatter3D(pred_sample[:, 0], pred_sample[:, 1], pred_sample[:, 2], alpha=0.3, s=20, c='blue')
            ax.scatter3D(np_d[:, 0], np_d[:, 1], np_d[:, 2], alpha=1, s=40, c='red')
            ax.set_title(title)
            ax.grid()
    ...
    def set_int_coords(self, data, gamma, sup_01):
        """
    ...
    def pi_hBeta_sampler(self, data, n_pts=2, a_0=1, gamma=0.1, sup_01=True):
        ...
        # divide the space - crucial to initialize
        timer.start()
        self.set_int_coords(data=data, gamma=gamma, sup_01=sup_01)
        ...
        # todo:
        #   this part here need to be computed as multiprocessing - Not easy to do
        node_ind = 0
        timer.start()
        ...
        # TODO:
        #   enable to run this code as 4d matrix
        #   this code needs to be written more efficiently - using matrix multiplication
        # phi_4d_mat = np.empty(shape=(self.L, self.I, n_parts, n_pts), dtype=np.float32)
        ...
        def _batches_pi_hBeta_sample(phi_sample, prod_ind_mat, n_pts, n_parts)
    ...
    def pred_map_sample(self, pi_map_sample, map_seg_pdist, n_samples=1000):
        ...
        pred_vec = np.sum(pi_cumsum < u_mat, axis=0)
